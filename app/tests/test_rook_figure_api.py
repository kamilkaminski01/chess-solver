...
class TestRookAPI(TestCase):
    def setUp(self):
        self.app = app.test_client()

    def test_list_moves_rook(self):
        response = self.app.get("/api/v1/rook/d4")
        data = response.get_json()

        self.assertEqual(response.status_code, 200)
        self.assertEqual(data["figure"], "rook")
    ...
    def test_list_moves_invalid_field(self):
        response = self.app.get("/api/v1/rook/h15")
        data = response.get_json()
    ...
    def test_validate_move_valid_move(self):
        response = self.app.get("/api/v1/rook/d4/d8")
        data = response.get_json()
    ...
    def test_validate_move_invalid_move(self):
        response = self.app.get("/api/v1/rook/d4/c6")
        data = response.get_json()
